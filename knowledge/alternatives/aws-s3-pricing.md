...
Dismiss
[Skip to main content](https://aws.amazon.com/s3/pricing/#aws-page-content-main)
...
There are per-request ingest charges when using PUT, COPY, or lifecycle rules to move data into any S3 storage class. Consider the ingest or transition cost before moving objects into any storage class. Estimate your costs using the [AWS Pricing Calculator](https://calculator.aws/). To find the best S3 storage class for your workload, learn more [here](https://aws.amazon.com/s3/storage-classes/).
...
**((4 bytes * 1024 dimensions) vector data/vector + 1 KB filterable metadata/vector + 1 KB non-filterable metadata/vector + 0.17 KB key/vector) = 6.17 KB logical storage per average vector  
...
**Total monthly PUT cost = 1 full upload/6 months * 59 GB total storage * $0.20/GB = $1.97
**S3 Vectors Query charge**
((4 bytes * 1024 dimensions) vector data/vector + 1 KB filterable metadata/vector + 0.17 KB key/vector) = 5.17 KB/average vector processed  
...
**S3 Vectors PUT charge  
**Total monthly PUT cost = 1 full upload/6 months * 2,354 GB total storage * $0.20/GB = $78.46 
**S3 Vectors Query charge  
**((4 bytes * 1024 dimensions) vector data/vector + 1 KB filterable metadata/vector + 0.17 KB key/vector) = 5.17 KB/average vector processed  
...
**Total cost** = $1,217.29 per month
  ...
  * You pay for all bandwidth into and out of Amazon S3, except for the following:  
    ...
    * EU customers may request reduced data transfer rates for eligible use cases under the European Data Act. Please contact [AWS Customer Support](https://aws.amazon.com/contact-us/) for more information.
The pricing below is based on data transferred "in" and "out" of Amazon S3 (over the public internet)†††. Learn more about [AWS Direct Connect pricing](https://aws.amazon.com/directconnect/pricing/).
For Data Transfers exceeding 500 TB/Month, [please contact us](https://pages.awscloud.com/ln_NAMER_AmazonS3-Contact-Us.html).   
...
For S3 Multi-Region Access Points availability in AWS Regions, please visit the [user guide](https://docs.aws.amazon.com/AmazonS3/latest/userguide/MultiRegionAccessPointRestrictions.html).
...
S3 Multi-Region Access Points failover controls let you shift S3 data access request traffic routed through an Amazon S3 Multi-Region Access Point within minutes to an alternate AWS Region to build highly available applications for business continuity. To use [failover controls](https://aws.amazon.com/s3/features/multi-region-access-points/), you are charged for S3 API costs to view the current routing control status of each Region and submit any routing control changes for initiating a failover. 
...
Check your performance with the Amazon S3 Transfer Acceleration [speed comparison tool](http://s3-accelerate-speedtest.s3-accelerate.amazonaws.com/en/accelerate-speed-comparsion.html).  
...
Region:
####  S3 bucket types
Amazon S3 supports four different [bucket types](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingBucket.html): general purpose buckets, directory buckets, and table buckets, and vector buckets. S3 general purpose buckets are available in all AWS Regions. For Regional availability details, visit the S3 User Guide for [directory buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/endpoint-directory-buckets-AZ.html), [table buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-tables-regions-quotas.html), and [vector buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-vectors-regions-quotas.html).
####  S3 general purpose buckets
[Amazon S3 general purpose buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingBucket.html) are the original S3 bucket type, and a single general purpose bucket can contain objects stored across all storage classes except S3 Express One Zone.  

####  S3 directory buckets
[Amazon S3 directory buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/directory-buckets-overview.html) only allow objects stored in the S3 Express One Zone storage class, which provides faster data processing within a single Availability Zone.  

####  S3 table buckets
[Amazon S3 table buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-tables.html) are purpose-built for storing tabular data. They deliver up to 3x faster query performance and up to 10x higher transactions per second, making them specifically optimized for analytics workloads.
Table buckets  | Free  
---|---  
####  S3 vector buckets
[Amazon S3 vector buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-vectors.html) are purpose-built for storing and querying vectors. Within a vector bucket, you use dedicated vector APIs to write vector data and query it based on semantic meaning and similarity.
Vector buckets  | Free  
---|---  
S3 Access Grants 
...
Storage management 
S3 Metadata pricing 
[Amazon S3 Metadata](https://aws.amazon.com/s3/features/metadata/) delivers queryable object metadata in near real time to organize your data and accelerate data discovery. This helps you to curate, identify, and use your Amazon S3 data for business analytics, real-time inference applications, and more.  
...
_Note that separate charges for table storage, maintenance, and requests will apply._  
...
* * *
Region:
Except as otherwise noted, our prices are exclusive of applicable taxes and duties, including VAT and applicable sales tax. For customers with a Japanese billing address, use of AWS is subject to Japanese Consumption Tax. To learn more, visit our [consumption tax FAQs »](https://aws.amazon.com/c-tax-faqs/)
...
Except as otherwise noted, our prices are exclusive of applicable taxes and duties, including VAT and applicable sales tax. For customers with a Japanese billing address, use of AWS is subject to Japanese Consumption Tax. To learn more, visit our [consumption tax FAQs](https://aws.amazon.com/c-tax-faqs/).
Amazon S3 storage usage is calculated in binary gigabytes (GB), where 1 GB is 230 bytes. This unit of measurement is also known as a gibibyte (GiB), defined by the International Electrotechnical Commission (IEC). Similarly, 1 TB is 240 bytes, i.e. 1024 GBs.
For Reduced Redundancy Storage pricing please visit the [S3 Reduced Redundancy detail page](https://aws.amazon.com/s3/reduced-redundancy/).
For S3 pricing examples, go to the [S3 billing FAQs](https://aws.amazon.com/s3/faqs/#Billing) or use the [AWS Pricing Calculator](http://aws.amazon.com/calculator/).
##  AWS Free Tier
...
Estimate the cost for your storage solution. Configure a cost estimate that fits your unique business or personal needs with Amazon S3. [Try out the AWS Pricing Calculator](https://calculator.aws/#/).
![Missing alt text value](https://d1.awsstatic.com/onedam/marketing-channels/website/aws/en_US/product-categories/compute/approved/images/calc.9fb92114ed7af2799dc8f88708109842e341e928.jpeg)
## Get started
[ FAQs Amazon S3 FAQs Learn more ](https://aws.amazon.com/s3/faqs/)
[ Getting started Get started with Amazon S3 Start building ](https://aws.amazon.com/s3/getting-started/)
[ Free tier Get started with Amazon S3 with AWS Free Tier Sign up ](https://aws.amazon.com/s3/pricing/?loc=ft#AWS_Free_Tier)
[ Features page Amazon S3 features Learn more ](https://aws.amazon.com/s3/features/)
[ Fast & Scalable Shared Storage  Accelerate compute-heavy workloads with Amazon FSx for Lustre  ![](https://d1.awsstatic.com/Digital%20Marketing/sitemerch/ups/Site-Merch_Storage-FSx-HDD_2up.33ef0afdc2a5a00b8055e995bbaaef06c2285467.png) ](https://aws.amazon.com/blogs/storage/supercharge-your-compute-workloads-with-amazon-fsx-for-lustre-and-amazon-s3/?sc_icampaign=Product_Launch_m3y20_psc_core-infra_storage_fsx-l-s3-blog_sm-2up&sc_ichannel=ha&sc_icontent=awssm-7319_pac&sc_ioutcome=CSI_Digital_Marketing&sc_iplace=2up&trk=ha_a134p0000078LWUAA2~ha_awssm-7319_pac&trkCampaign=psc_core-infra_storage_fsx-l-s3_re:Invent-20_blog)
[ AWS Free Tier  Gain free, hands-on experience with AWS for 12 months  ![](https://d1.awsstatic.com/Digital%20Marketing/sitemerch/ups/Site-Merch_Free-Tier-2up.800bbc9e8f08968b99b97bde4372f203feb1a584.png) ](https://aws.amazon.com/free/?sc_icontent=awssm-evergreen-free_tier&sc_iplace=2up&trk=ha_awssm-evergreen-free_tier&sc_ichannel=ha&sc_icampaign=evergreen-free_tier)
[ Proven Security to Accelerate Your Innovation  Find the resources you need to build, run, and scale your applications on the most secure cloud computing environment available today »  ![](https://d1.awsstatic.com/Digital%20Marketing/sitemerch/ups/Site-Merch_AWS-Security_1up.8458f6fcf954862f61081dd3ce06ac00a789481e.png) ](https://aws.amazon.com/products/security/resources/?sc_icampaign=aware_GC-200-BDM-launch-content-hub-security-resources&sc_ichannel=ha&sc_icontent=awssm-10848_gc&sc_iplace=1up&trk=cec45ed7-d461-4cea-a558-3582bf3bb2c9~ha_awssm-10848_gc)
[ Amazon S3](https://aws.amazon.com/s3/?nc=sn&loc=0)
[ Overview](https://aws.amazon.com/s3/?nc=sn&loc=1) [ Features ](https://aws.amazon.com/s3/pricing/) [ Storage classes ](https://aws.amazon.com/s3/pricing/) [ Pricing](https://aws.amazon.com/s3/pricing/?nc=sn&loc=4) [ Security](https://aws.amazon.com/s3/security/?nc=sn&loc=5) [ Resources ](https://aws.amazon.com/s3/pricing/) [ FAQs](https://aws.amazon.com/s3/faqs/?nc=sn&loc=7)
CCBA-Footer
...
[x](https://twitter.com/awscloud)[facebook](https://www.facebook.com/amazonwebservices)[linkedin](https://www.linkedin.com/company/amazon-web-services/)[instagram](https://www.instagram.com/amazonwebservices/)[twitch](https://www.twitch.tv/aws)[youtube](https://www.youtube.com/user/AmazonWebServices/Cloud/)[podcasts](https://aws.amazon.com/podcasts/?nc1=f_cc)[email](https://pages.awscloud.com/communication-preferences?trk=homepage)
  ...
  * [Your Privacy ChoicesCalifornia Consumer Privacy Act (CCPA) Opt-Out Icon](https://aws.amazon.com/s3/pricing/)
  * [Cookie Preferences](https://aws.amazon.com/s3/pricing/)


© 2025, Amazon Web Services, Inc. or its affiliates. All rights reserved.
Close [ S3 Intelligent-Tiering](https://aws.amazon.com/s3/storage-classes/intelligent-tiering/) [ S3 Express One Zone](https://aws.amazon.com/s3/storage-classes/express-one-zone/) [ S3 Glacier](https://aws.amazon.com/s3/storage-classes/glacier/) [ S3 Glacier Instant Retrieval](https://aws.amazon.com/s3/storage-classes/glacier/instant-retrieval/) [ More storage classes](https://aws.amazon.com/s3/storage-classes/?nc=sn&loc=3)
Close [ Getting started](https://aws.amazon.com/s3/getting-started/?nc=sn&loc=6&dn=1) [ Videos](https://aws.amazon.com/s3/videos/?nc=sn&loc=6&dn=2) [ Customers](https://aws.amazon.com/s3/customers/?nc=sn&loc=6&dn=3) [ More resources](https://aws.amazon.com/s3/resources/?nc=sn&loc=6)
Close [ Access Grants](https://aws.amazon.com/s3/features/access-grants/?nc=sn&loc=2&dn=1) [ Access Points](https://aws.amazon.com/s3/features/access-points/?nc=sn&loc=2&dn=1) [ Batch Operations](https://aws.amazon.com/s3/features/batch-operations/?nc=sn&loc=2&dn=2) [ Block Public Access](https://aws.amazon.com/s3/features/block-public-access/?nc=sn&loc=2&dn=3) [ Cost Optimization](https://aws.amazon.com/s3/cost-optimization/?nc=sn&loc=2&dn=4) [ Metadata](https://aws.amazon.com/s3/features/metadata/) [ Mountpoint](https://aws.amazon.com/s3/features/mountpoint/?nc=sn&loc=2) [ Multi-Region Access Points](https://aws.amazon.com/s3/features/multi-region-access-points/?nc=sn&loc=2&dn=5) [ Object Lambda](https://aws.amazon.com/s3/features/object-lambda/?nc=sn&loc=2&dn=5) [ Object Lock](https://aws.amazon.com/s3/features/object-lock/) [ Replication](https://aws.amazon.com/s3/features/replication/?nc=sn&loc=2&dn=5) [ Storage Lens](https://aws.amazon.com/s3/storage-lens/) [ Storage Browser](https://aws.amazon.com/s3/features/storage-browser/) [ S3 on Outposts](https://aws.amazon.com/s3/outposts/?nc=sn&loc=2&dn=7) [ Tables](https://aws.amazon.com/s3/features/tables/) [ More features](https://aws.amazon.com/s3/features/?nc=sn&loc=2)
